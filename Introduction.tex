...
\begin{itemize}
\item \url http://www.debian.org/releases/ 
\item \textit{stable} contient la dernière distribution officiellement sortie de Debian (squeeze)
\item \textit{testing} contient les paquets qui n'ont pas encore été acceptés dans la distribution stable, mais qui sont en attente de l'être. (wheezy)
\textit{unstable}  est celle sur laquelle les activités de développement se déroulent (Sid)
\end{itemize}
\end{frame}
